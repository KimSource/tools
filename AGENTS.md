...
- 도구는 Registry를 통해 lazy-load하며, 다른 도구를 직접 import하지 않는다. 계산·변환 로직은 UI와 분리한다.
- 사용자 데이터는 기기 안에서 처리하며, 서버 전송이나 자동 영구 저장을 추가하지 않는다.
- 도구 UI의 저장소·파일·클립보드 접근은 공통 서비스를 통한다. 실제로 필요한 서비스만 만든다.
- UI 문자열은 ko/en i18n key로 관리하고, 자체 design tokens와 라이트/다크·모바일·키보드 접근성을 유지한다.
...
- 코드 변경 시 core 동작은 Vitest로, 주요 브라우저 흐름은 Playwright로 검증한다. PWA 변경은 production build에서 오프라인·업데이트를 확인한다.
